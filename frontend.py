...
def normalize_row(r):
    """
    Normalize rows returned from backend.get_electricity_records().
    The backend ALWAYS returns:
        (record_id, country_name, year, population, people_without, people_with)
    """
    res = {
        "id": None,
        "country": None,
    ...
    # Handle rows with at least 6 values
    if len(r) >= 6:
        res["id"] = r[0]
        res["country"] = r[1]
        res["year"] = r[2]
    ...
    for key in ("population", "without", "with"):
        val = res[key]
        if val is None:
            continue
        try:
...
def screen_update():
    safe_clear(main_frame)
    ctk.CTkLabel(main_frame, text="Update Record", font=("Helvetica", 20)).pack(pady=10)

    frm = ctk.CTkFrame(main_frame)
    frm.pack(pady=10)

    ctk.CTkLabel(frm, text="Record ID:").grid(row=0, column=0, padx=8, pady=6)
    ent_id = ctk.CTkEntry(frm, width=200)
    ent_id.grid(row=0, column=1)

    ctk.CTkLabel(frm, text="People WITHOUT:").grid(row=1, column=0, padx=8, pady=6)
    ent_without = ctk.CTkEntry(frm, width=200)
    ent_without.grid(row=1, column=1)

    ctk.CTkLabel(frm, text="People WITH:").grid(row=2, column=0, padx=8, pady=6)
    ent_with = ctk.CTkEntry(frm, width=200)
    ent_with.grid(row=2, column=1)

    ctk.CTkLabel(frm, text="Population (optional):").grid(row=3, column=0, padx=8, pady=6)
    ent_pop = ctk.CTkEntry(frm, width=200)
    ent_pop.grid(row=3, column=1)
    ...
    def load_record():
        ...
        ent_without.delete(0, "end")
        ent_without.insert(0, "" if match["without"] is None else match["without"])

        ent_with.delete(0, "end")
        ent_with.insert(0, "" if match["with"] is None else match["with"])

        ent_pop.delete(0, "end")
        ent_pop.insert(0, "" if match["population"] is None else match["population"])

    ctk.CTkButton(frm, text="Load", width=100, command=load_record).grid(row=0, column=2, padx=8)

    def submit_update():
        try:
            rid = int(ent_id.get())
        except:
            messagebox.showerror("Error", "Record ID must be numeric.")
            return
        ...
        try:
            without = parse_int(ent_without.get())
            with_ = parse_int(ent_with.get())
            pop = parse_int(ent_pop.get())
        except Exception:
            messagebox.showerror("Error", "Values must be numbers.")
            return

        try:
            backend.update_electricity_record(rid, without=without, with_people=with_, population=pop)
            messagebox.showinfo("Updated", "Record updated successfully.")
            screen_view()
        except Exception as e:
            messagebox.showerror("Error", str(e))

    ctk.CTkButton(main_frame, text="Update", image=icons.get("add"), width=180, command=submit_update).pack(pady=15)
...
def do_yearly_trend():
    try:
        rows = backend.query_yearly_trend()
    except Exception as e:
        messagebox.showerror("Query Error", str(e))
        rows = []
    proc = []
...
add_nav_button("Update Record", screen_update, 330, icons.get("add"))  # uses add.png as requested
add_nav_button("Queries", screen_queries, 400, icons.get("query"))
